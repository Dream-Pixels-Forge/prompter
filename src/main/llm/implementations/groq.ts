...
export const groqImpl: ProviderImplementation = {
  ...
  async generate({ model, prompt, apiKey, endpoint, signal }) {
    const baseUrl = (endpoint ?? 'https://api.groq.com/openai/v1').replace(/\/+$/, '');
    ...
    try {
      ...
      if (content === undefined || content === null) {
        throw new Error('Groq API error: no choices returned');
      }

      return content;
    } catch (err: unknown) {
      if (err instanceof DOMException && (err.name === 'TimeoutError' || err.name === 'AbortError')) {
        throw new Error('Groq API error: request timed out after 60s');
      }
      throw err;
    }
  },
