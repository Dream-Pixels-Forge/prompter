...
export const mistralImpl: ProviderImplementation = {
  providerId: 'mistral',
  ...
  async generate({ model, prompt, apiKey, endpoint, signal }) {
    ...
    try {
      ...
      if (content === undefined || content === null) {
        throw new Error('Mistral API error: no choices returned');
      }

      return content;
    } catch (err: unknown) {
      if (err instanceof DOMException && (err.name === 'TimeoutError' || err.name === 'AbortError')) {
        throw new Error('Mistral API error: request timed out after 60s');
      }
      throw err;
    }
  },
